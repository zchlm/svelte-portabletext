import type { ToolkitNestedPortableTextSpan, ToolkitPortableTextList, ToolkitPortableTextListItem } from '@portabletext/toolkit';
import type { ArbitraryTypedObject, PortableTextBlock, PortableTextBlockStyle, PortableTextListItemType, PortableTextMarkDefinition } from '@portabletext/types';
import type { Component, Snippet } from 'svelte';
import type { InputValue, NormalizedBlocks, PortableTextMarkType, PortableTextSvelteContext } from './ptTypes';
export type NodeType = 'block' | 'mark' | 'blockStyle' | 'listStyle' | 'listItemStyle';
export type MissingComponentHandler = (message: string, context: {
    type: string;
    nodeType: NodeType;
}) => void;
...
export interface CustomBlockComponentProps<BlockType extends Partial<ArbitraryTypedObject> = ArbitraryTypedObject, ContextType = PortableTextSvelteContext> extends CommonComponentProps<ContextType> {
    value: BlockType;
    isInline: boolean;
    /**
...
export interface BlockComponentProps<ContextType = PortableTextSvelteContext> extends CommonComponentProps<ContextType> {
    value: PortableTextBlock;
}
export interface ListItemComponentProps<ContextType = PortableTextSvelteContext> extends CommonComponentProps<ContextType> {
    value: ToolkitPortableTextListItem;
}
export interface ListComponentProps<ContextType = PortableTextSvelteContext> extends CommonComponentProps<ContextType> {
    value: ToolkitPortableTextList;
}
/**
 * Values passed under the `portableText` prop to components rendering marks.
 */
export interface MarkComponentProps<MarkType = PortableTextMarkDefinition, ContextType = PortableTextSvelteContext> extends CommonComponentProps<ContextType>, Pick<ToolkitNestedPortableTextSpan, 'markKey' | 'markType'>, Pick<CustomBlockComponentProps, 'parentBlock'> {
    plainTextContent: string;
    value: MarkType extends string ? undefined : MarkType;
}
export type CustomStyles = Record<PortableTextBlockStyle, BlockComponent | undefined> | BlockComponent;
...
export type CustomListTypes = Record<PortableTextListItemType, ListComponent | undefined> | ListComponent;
export type CustomListItemTypes = Record<PortableTextListItemType, ListItemComponent | undefined> | ListItemComponent;
...
export type CustomBlockComponent = Component<{
    portableText: CustomBlockComponentProps;
    children?: Snippet;
}> | Component<any>;
export type MarkComponent = Component<{
...
export type ListItemComponent = Component<{
